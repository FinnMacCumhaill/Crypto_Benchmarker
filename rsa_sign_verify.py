...
def sign_verify_times(private_key, public_key, message):
    times_sign = []
    times_verify = []
...
for key_size in rsa_key_sizes:
    ...
    times_sign, times_verify = sign_verify_times(private_key, public_key, message)

    # Calculate and print the average times
    average_sign_time = sum(times_sign) / 10
    average_verify_time = sum(times_verify) / 10
